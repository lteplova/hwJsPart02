...
function renderColor(colorContainerEl, data, index) {
  ...
  divEl.innerHTML = `<div class="tooltip">${data.title}</div>
    <input quickbeam="color" id="swatch-${index}-${
    data.type
  }" type="radio" name="color" onChange="changeStorage(this, 'color')" value="${
    data.type
  }" ${checked} ${data.isAvailable ? "" : "disabled"}>
    <label for="swatch-${index}-${data.type}" style="border-color: ${
    data.type
  };">
...
function renderSize(sizeContainerEl, data, index) {
  ...
  divEl.innerHTML = `<input id="swatch-${index}-${
    data.type
  }" type="radio" name="size" onChange="changeStorage(this, 'size')" value="${
    data.type
  }" ${checked} ${data.isAvailable ? "" : "disabled"}>
...
function renderCart(cartContainerEl, data, index) {
  cartContainerEl.innerHTML = `<div class="quick-cart-product quick-cart-product-static" id="quick-cart-product-${
    data.id
  }" style="opacity: 1;">
